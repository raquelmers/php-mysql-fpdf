...
INSERT INTO `dependants` (`dependant_id`, `first_name`, `last_name`, `relationship`, `employee_id`) VALUES
...
--
-- Stand-in structure for view `detail_dependants`
-- (See below for the actual view)
--
CREATE TABLE `detail_dependants` (
`fullnameDependant` varchar(101)
,`relationship` varchar(25)
,`fullnameEmployee` varchar(46)
);

-- --------------------------------------------------------

--
-- Stand-in structure for view `detail_employees`
-- (See below for the actual view)
--
CREATE TABLE `detail_employees` (
`fullname` varchar(46)
...
--
-- Stand-in structure for view `spots`
-- (See below for the actual view)
--
CREATE TABLE `spots` (
...
--
-- Structure for view `detail_dependants`
--
DROP TABLE IF EXISTS `detail_dependants`;

CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `detail_dependants`  AS SELECT concat(`dependants`.`first_name`,' ',`dependants`.`last_name`) AS `fullnameDependant`, `dependants`.`relationship` AS `relationship`, concat(`employees`.`first_name`,' ',`employees`.`last_name`) AS `fullnameEmployee` FROM (`dependants` join `employees` on(`dependants`.`employee_id` = `employees`.`employee_id`)) ;

-- --------------------------------------------------------

--
-- Structure for view `detail_employees`
--
DROP TABLE IF EXISTS `detail_employees`;

CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `detail_employees`  AS SELECT concat(`employees`.`first_name`,' ',`employees`.`last_name`) AS `fullname`, `jobs`.`job_title` AS `job_title`, `departments`.`department_name` AS `department_name` FROM ((`departments` join `employees` on(`departments`.`department_id` = `employees`.`department_id`)) join `jobs` on(`employees`.`job_id` = `jobs`.`job_id`)) ;

-- --------------------------------------------------------

--
-- Structure for view `spots`
--
DROP TABLE IF EXISTS `spots`;

CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `spots`  AS SELECT `regions`.`region_name` AS `region_name`, `countries`.`country_name` AS `country_name`, `locations`.`state_province` AS `state_province`, `locations`.`city` AS `city`, `locations`.`street_address` AS `street_address` FROM ((`regions` join `countries` on(`regions`.`region_id` = `countries`.`region_id`)) join `locations` on(`countries`.`country_id` = `locations`.`country_id`)) ;
...
--
ALTER TABLE `dependants`
  MODIFY `dependant_id` int(11) NOT NULL AUTO_INCREMENT, AUTO_INCREMENT=31;
...
--
ALTER TABLE `dependants`
  ADD CONSTRAINT `dependants_ibfk_1` FOREIGN KEY (`employee_id`) REFERENCES `employees` (`employee_id`) ON DELETE CASCADE ON UPDATE CASCADE;
